...
### 🎯 智能测评
- **自适应题目推荐**: 基于BKT算法、知识图谱RAG引擎和盘古7B模型，根据学生掌握度智能推荐题目
- **多级题目选择策略**: 优先使用知识图谱RAG检索，支持多级SQL备用方案，确保题目推荐成功率
- **题目互选机制**: 使用盘古7B模型从候选题目中选择最合适的题目，考虑知识点关联和难度匹配
- **细粒度知识点追踪**: 支持知识点大类和小类两级精细追踪，每个知识点独立维护BKT参数
- **实时掌握度更新**: 使用贝叶斯知识追踪（BKT）算法实时更新学生掌握度，支持个性化参数调整
- **AI答案评估**: 使用盘古7B模型进行智能答案评估，提供详细反馈和解析，支持答案评估缓存优化

### 📊 学习分析
- **学生画像生成**: 自动生成个性化的学生学习画像，包含掌握度分布、答题统计等信息
- **薄弱点识别**: 基于BKT算法自动识别学生薄弱知识点（掌握度低于阈值），提供针对性建议
- **学习模式分析**: 分析答题速度、错误模式、进步趋势、知识点关联等学习行为
- **可视化报告**: 生成详细的学习分析报告，包含雷达图、掌握度柱状图、答题历史等可视化图表
- **历史记录查询**: 支持按知识点、时间范围等条件查询历史答题记录

### 🕸️ 知识图谱
- **自动构建与更新**: 使用盘古7B模型自动分析题目，提取实体关系，构建全局知识图谱
- **智能更新检测**: 通过数据库哈希自动检测题库变化，智能重建知识图谱
- **关系网络可视化**: 展示题目、知识点大类、知识点小类和难度之间的关系网络
- **2D/3D可视化**: 支持2D和3D两种可视化模式，提供更丰富的视觉体验
- **多种布局算法**: 支持spring、circular、kamada_kawai等多种图布局算法
- **交互式探索**: 基于Plotly的交互式知识图谱可视化，支持缩放、拖拽、节点筛选等操作
- **知识图谱RAG**: 基于知识图谱的检索增强生成，实现更精准的题目推荐

### 👥 用户管理
- **用户注册登录**: 支持学生和教师两种角色的注册和登录，密码采用SHA256哈希存储
- **角色权限分离**: 学生和教师拥有不同的功能界面和权限
- **用户信息管理**: 支持真实姓名、用户名等用户信息管理
- **默认账号支持**: 系统提供默认测试账号，方便快速体验

### ⚙️ 系统管理
- **题库管理**: 支持通过UI界面添加题目，支持JSON格式题目批量导入和数据库迁移
- **题目搜索**: 支持按知识点大类、小类搜索题目，查看题目详情和统计信息
- **学生管理**: 教师可查看所有学生列表，查看学生详细学习数据和答题情况
- **系统监控**: 实时监控系统状态，包括题目数量、用户数量、数据库状态等统计信息
- **知识图谱管理**: 支持手动重建知识图谱，查看知识图谱构建状态
- **数据库管理**: 基于SQLite的数据库管理，支持用户、题目、答题记录、学生状态等完整数据存储
