...
SMART_QUESTION_CONFIG = {
    ...
    "question_radar_default": 0.5, # 雷达图默认难度/掌握度（无数据时）
        ...
        # 知识图谱相关配置
    "rebuild_kg": False,  # 是否强制重建知识图谱（默认使用缓存）
    "kg_cache_path": DATA_DIR / "knowledge_graph.pkl",  # 知识图谱缓存路径
    "kg_analysis_depth": 2,  # 知识图谱分析深度
    
    # RAG配置
    "kg_rag_top_k": 5,  # 知识图谱RAG返回的候选题目数量
    "use_kg_rag": True,  # 是否使用知识图谱RAG（关闭则仅使用SQL）
}
...
SYSTEM_INFO = {
    ...
    "description": "基于知识图谱和RAG的智能个性化出题系统",
    "model": "openPanGu-Embedded-7B-V1.1",
    "device": "Ascend 910B NPU",
    "architecture": "知识图谱 + RAG + BKT",
    "core_features": [
        "盘古7B构建全局知识图谱",
        "基于知识图谱的RAG检索",
        "盘古7B智能答案判断",
